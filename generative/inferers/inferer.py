...
class DiffusionInferer(Inferer):
    ...
    def sample(
        ...
        Args:
            ...
            class_labels: Class labels for network input.
            mode: Conditioning mode for the network.
...
class TransformDiffusionInferer(DiffusionInferer):
    ...
    def __call__(
            self,
            abnormal_inputs: torch.Tensor,
            normal_inputs: torch.Tensor,
            encoder_model: Callable[..., torch.Tensor],
            disentangle_model: Callable[..., torch.Tensor],
            transform_model: Callable[..., torch.Tensor],
            noise: torch.Tensor,
            timesteps: torch.Tensor,
            sampled_concept: numpy.ndarray,
    ):
        z_emb_pt = encoder_model(abnormal_inputs)
        noisy_abnormal_image = self.scheduler.add_noise(original_samples=abnormal_inputs, noise=noise,
                                                        timesteps=timesteps)

        model_output_pt = disentangle_model(
            x=noisy_abnormal_image, timesteps=timesteps, represent=z_emb_pt, sampled_concept=sampled_concept
        )

        noisy_normal_image = self.scheduler.add_noise(original_samples=normal_inputs, noise=noise, timesteps=timesteps)

        _, z_emb_ad_related = torch.chunk(z_emb_pt, chunks=2, dim=1)
        model_output_nc = transform_model(
            x=noisy_normal_image, timesteps=timesteps, represent=z_emb_ad_related,
        )

        return z_emb_pt, noisy_abnormal_image, model_output_pt, noisy_normal_image, model_output_nc
