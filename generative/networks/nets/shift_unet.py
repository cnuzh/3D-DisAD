...
__all__ = ["ShiftUNetModel", "Return", "Return_grad", "return_wrap"]
...
def return_wrap(inp, coef=None):
    if isinstance(inp, Return):
        return inp.pred
    elif isinstance(inp, Return_grad):
        return inp.pred + coef * inp.grad
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, emb: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        time_emb = self.time_emb_proj(self.nonlinearity(emb))
        while len(time_emb.shape) < len(h.shape):
            time_emb = time_emb[..., None]

        if self.with_resblock_cond:
            cond_emb = self.cond_emb_proj(self.nonlinearity(context))
            while len(cond_emb.shape) < len(h.shape):
...
class ShiftUNetModel(nn.Module):
    """
    ...
    Args:
        ...
        use_flash_attention: if True, use flash attention for a memory efficient attention mechanism.
        dropout_cattn: if different from zero, this will be the dropout value for the cross-attention layers
        with_shift: if True, add shift layers to the model.
    """
    ...
    def __init__(
        ...
        if with_conditioning is True and cross_attention_dim is None:
            raise ValueError(
                "PDAEModelUNet expects dimension of the cross-attention conditioning (cross_attention_dim) "
                "when using with_conditioning."
            )

        # if with_conditioning is True and num_class_embeds is None:
        #     raise ValueError(
        #         "PDAEModelUNet expects dimension of the class conditioning (num_class_embeds) "
        #         "when using with_conditioning."
        #     )

        if cross_attention_dim is not None and with_conditioning is False:
            raise ValueError(
                "PDAEModelUNet expects with_conditioning=True when specifying the cross_attention_dim."
            )

        if with_shift is True and latent_dims is None:
            raise ValueError(
                "PDAEModelUNet expects dimension of the representation conditioning (latent_dim) "
                "when using with_shift."
            )
        ...
        if with_shift is True and ddpm_path is None:
            raise ValueError(
                "`ddpm_path` should not be None when with_shift is True. "
            )
        ...
        # shift embedding
        if self.with_shift:
            self.shift_embedding = nn.Linear(self.latent_dims, self.time_embed_dim, bias=False)
    ...
    def init_model(self):
        if self.with_shift:
            # load ddpm weight
            state_dict = torch.load(self.ddpm_path, map_location="cpu")
            model_dict = dict()
    ...
    def train(self, mode=True):
        if self.with_shift:
            self.training = mode
            self.shift_embedding.train(mode)
    ...
    def parameters(self, recurse: bool = True):
        if self.with_shift:
            params = [
                self.shift_embedding.parameters(recurse),
    ...
    def forward(
        ...
        Args:
            ...
            represent: represent tensor (N, RepresentDim).
        ...
        """
        with torch.no_grad() if self.with_shift else contextlib.nullcontext():
            # 1. time
        ...
        if self.with_shift:
            # represent embed
            shift_emb = self.shift_embedding(represent)

            # 5. shift mid
            shift_h = self.shift_middle_block(hidden_states=h, temb=emb, context=shift_emb)
            ...
            for shift_upsample_block in self.shift_up_blocks:
                ...
                shift_h = shift_upsample_block(
                    hidden_states=shift_h, res_hidden_states_list=res_samples, temb=emb, context=shift_emb
                )

            # 7. shift output
            shift_grad = self.shift_out(shift_h)
        ...
        if self.with_shift:
            return Return_grad(pred=pred, grad=shift_grad)
