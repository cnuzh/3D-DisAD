...
__all__ = ["AutoencoderKL", "FeatureEncoder"]
...
class FeatureEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.quant_linear_mu = nn.Linear(int(latent_channels * (16 ** spatial_dims)), latent_channels)
        self.quant_conv_log_sigma = Convolution(
            spatial_dims=spatial_dims,
            in_channels=latent_channels,
            out_channels=latent_channels,
        ...
        self.quant_linear_log_sigma = nn.Linear(int(latent_channels * (16 ** spatial_dims)), latent_channels)
    ...
    def encode(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        z_mu = self.quant_conv_mu(h)
        z_mu = self.quant_linear_mu(torch.flatten(z_mu, 1))
        z_log_var = self.quant_conv_log_sigma(h)
        z_log_var = self.quant_linear_log_sigma(torch.flatten(z_log_var, 1))
    ...
    def reparameterize(self, z_mu: torch.Tensor, z_sigma: torch.Tensor) -> torch.Tensor:
        eps = torch.randn_like(z_sigma)
        z_vae = z_mu + eps * z_sigma
        return z_vae
...
if __name__ == '__main__':
    ...
    print(model.encoder)
    input = torch.randn((5, 1, 64, 64, 64,))
